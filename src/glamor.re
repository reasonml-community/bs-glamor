...
[@bs.module "glamor"] external makeCSS : styleObj => string = "css";

[@bs.module "glamor"] [@bs.scope "css"]
external makeGlobalCSS : (string, styleObj) => unit = "global";

[@bs.module "glamor"] [@bs.scope "css"]
external makeKeyFrames : Js.Dict.t(styleObj) => string = "keyframes";
...
let rec addDeclaration = (obj, decl: declaration) =>
  ...
  | Selector(sel, decls) =>
    addObjToStyles(obj, sel, declarationsToObj(decls))
  }
and declarationsToObj = decls =>
  List.fold_left(addDeclaration, emptyObj, decls);

let css = decls => makeCSS(declarationsToObj(decls));

let global = (selector, declarations) =>
  makeGlobalCSS(selector, declarationsToObj(declarations));

let keyframes = frames =>
  makeKeyFrames(
    Js.Dict.fromList(
      List.map(((k, v)) => (k, declarationsToObj(v)), frames),
    ),
  );
